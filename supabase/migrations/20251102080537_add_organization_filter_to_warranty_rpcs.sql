/*
  # Add Organization Filter to Warranty RPC Functions
  
  1. Changes
    - Add p_organization_id parameter to get_warranties_optimized
    - Add p_organization_id parameter to get_warranties_simple
    - Allow master users to query any organization
    - Regular users query their own organization by default
    
  2. Security
    - Master users can view any organization's warranties
    - Other users can only view their own organization
*/

-- Drop and recreate get_warranties_optimized with organization_id parameter
...
BEGIN
  ...
  IF p_organization_id IS NOT NULL THEN
    -- Verify user is master or querying their own org
    IF v_user_role = 'master' THEN
      v_target_org_id := p_organization_id;
    ELSIF p_organization_id = v_user_org_id THEN
      v_target_org_id := p_organization_id;
    ELSE
      -- User trying to access another org without permission
...
-- Also update get_warranties_simple
